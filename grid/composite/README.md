...
| 나우캐스트 | 지금 창이 완결된 시차만 쓰는 같은 절차를 다시 rolling-origin 으로 돌린 **나우캐스트 앙상블**의 등급이 A·B 이고, 대상 기간 기준 15개월 안에 타깃에 해당하는 구조 변화(생산능력·상장·지분법 등 매출 범위와 무관한 유형 제외)가 없을 때만 공개. 구간 = 그 앙상블 OOS 절대오차의 분할 conformal 80%. 최근 비교값이 최초 공시와 2% 넘게 다르면 레벨 환산 생략. 비공개 기업은 예측 수치를 출력하지 않는다. |
